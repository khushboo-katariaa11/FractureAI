...
export const generatePDFReport = async (
  ...
  // Description (matching your Streamlit exam description)
  yPos += 10;
  pdf.setFont('helvetica', 'bold');
  pdf.text('Description:', 20, yPos);
  pdf.setFont('helvetica', 'normal');
  yPos += 6;
  const description = patientData.clinicalNote || examDesc;
  const splitDesc = pdf.splitTextToSize(description, pageWidth - 40);
...
export const generateAdvancedPDFReport = async (
  ...
  reportElement.innerHTML = `
    ...
    <div style="margin-bottom: 25px;">
      <h3 style="font-size: 16px; font-weight: bold; color: #1f2937; margin-bottom: 15px;">Clinical Notes</h3>
